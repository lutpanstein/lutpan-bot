...
async def member_ban(client: Client, message: Message):
    ...
    if not bot.can_restrict_members:
        return await Man.edit("`Bejir kata lutpan lu ga ngetmin disini'")
    if not user_id:
        return await Man.edit("Gaketemu orangnya.")
    if user_id == client.me.id:
        return await Man.edit("**ANAK KONTOL MAU BAN LUTPAN**.")
    if user_id in DEVS:
    ...
    msg = (
        f"**MAKHLUK YANG DIBANNED :** {mention}\n"
        f"**NI MAKHLUK YANG JADI PELAKU :** {message.from_user.mention if message.from_user else 'Anon'}\n"
    )
    if message.command[0][0] == "d":
        await message.reply_to_message.delete()
    if reason:
        msg += f"**NI GARA² NYA :** {reason}"
